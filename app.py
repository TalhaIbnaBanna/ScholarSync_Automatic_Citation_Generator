...
# ── Global CSS: make component iframes transparent so they match the theme ──
st.markdown(
    """
    <style>
    /* Remove white background from ALL iframes (Streamlit component iframes) */
    iframe {
...
@st.cache_data(show_spinner=False, ttl=3600)
def _sanitize_scraped_author(raw_author: str) -> str:
    """Clean a raw author string scraped from HTML.
    - Truncates at special characters that signal the name has ended
    - Strips digits, affiliation markers, and common artefacts
    - Removes duplicate author names (case-insensitive)
    - Rejects tokens that look like dates, roles, or non-name words
    Returns 'Unknown Author' if nothing usable remains."""
    if not raw_author or not isinstance(raw_author, str):
        return "Unknown Author"

    # --- Phase 1: Truncate at boundary characters that signal end-of-name ---
    # Characters like : | / — – signal metadata after the author name
    # e.g. "John Smith | March 4, 2026" or "By Jane Doe: Science Reporter"
    raw_author = raw_author.split("\n")[0]  # Only keep first line
    raw_author = re.split(r"[:|/\u2014\u2013\u2022\t]", raw_author)[0]  # — – • and tab

    # Remove common prefixes like "By", "Written by", "Author:", etc.
    raw_author = re.sub(r"^\s*(Written\s+by|Reported\s+by|By)\s+", "", raw_author, flags=re.IGNORECASE)

    # --- Phase 2: Split on multi-author separators ---
    separators = [", and ", " and ", " & ", ";"]
    parts = [raw_author]
    for sep in separators:
        if sep in parts[0]:
            parts = [p.strip() for p in parts[0].split(sep) if p.strip()]
            break

    # Words that are NOT author names (roles, dates, organizations, etc.)
    NON_NAME_WORDS = {
        "reporter", "editor", "correspondent", "staff", "writer", "contributor",
        "journalist", "columnist", "associated", "press", "reuters", "updated",
        "published", "modified", "january", "february", "march", "april", "may",
        "june", "july", "august", "september", "october", "november", "december",
        "jan", "feb", "mar", "apr", "jun", "jul", "aug", "sep", "oct", "nov", "dec",
        "am", "pm", "est", "pst", "gmt", "utc", "cst", "cet",
        "mon", "tue", "wed", "thu", "fri", "sat", "sun",
        "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday",
    }

    cleaned_parts = []
    seen_lower: set = set()  # Track already-added names (lowercase) for dedup
    for name in parts:
        # Remove digits (affiliation markers such as "Smith1,2")
        name = re.sub(r"\d+", "", name)
        # Remove stray superscript-style characters and common punctuation debris
        name = re.sub(r"[*†‡§¶#\\^@!?$%={}()\[\]]+", "", name)
        # Remove trailing commas and punctuation that aren't part of a name
        name = re.sub(r"[,;:.]+$", "", name)
        # Collapse multiple spaces that remain after stripping
        name = re.sub(r"\s{2,}", " ", name)
        name = name.strip(" ,;:.")

        # Skip if nothing meaningful left (must contain at least one letter)
        if not name or not re.search(r"[a-zA-Z]", name):
            continue

        # Skip if any word is a known non-name word (date, role, etc.)
        name_words = name.lower().split()
        if any(w in NON_NAME_WORDS for w in name_words):
            continue

        # Skip if the "name" is too long (likely grabbed extra text)
        if len(name) > 60:
            continue

        # Skip if the name has too many words (real names rarely exceed 5 words)
        if len(name_words) > 5:
            continue
    ...
    def fetch():
        ...
        for getter in [
            ...
            try:
                ...
                continue
        # Sanitize: strip numbers, remove duplicates, clean artefacts
        author = _sanitize_scraped_author(author)
...
def get_parenthetical_citation(metadata: Dict[str, Any], style_name: str) -> str:
    ...
    try:
        ...
        # Get the first author's name (before any comma separating multiple authors)
        if "," in author_full:
            first_author = author_full.split(",")[0].strip()
        else:
            first_author = author_full.strip()
        # Extract last name (last word of "FirstName LastName")
        parts = first_author.split()
        last_name = parts[-1] if parts else "Unknown"
        # Add "et al." if there are multiple authors
        if "," in author_full or " and " in author_full.lower() or " & " in author_full:
...
def _extract_identifier_from_url(url: str) -> Optional[Tuple[str, str]]:
    ...
    # ScienceDirect PII links: extract the PII and resolve to DOI via Crossref
    # e.g. https://www.sciencedirect.com/science/article/abs/pii/S1059056020303233
    pii_match = re.search(r"sciencedirect\.com/science/article/(?:abs/)?pii/(S\d{15,20})", url, re.IGNORECASE)
    if pii_match:
...
def _resolve_pii_to_doi(pii: str) -> Optional[str]:
    """Resolve an Elsevier PII (Publisher Item Identifier) to a DOI via Crossref.
    ScienceDirect URLs contain PIIs like S1059056020303233 which Crossref indexes
    as 'alternative-id' fields. Returns the DOI string or None."""
    try:
        resp = requests.get(
            "https://api.crossref.org/works",
            params={"query": pii, "rows": 3},
            headers={"User-Agent": "ScholarSync/1.0 (mailto:scholarsync@example.com)"},
            timeout=15,
        )
        if resp.status_code != 200:
            return None
        ...
        for item in items:
            ...
            # Confirm the PII matches an alternative-id to avoid false positives
            if pii in alt_ids:
                doi = item.get("DOI", "")
                if validate_doi(doi):
                    return doi
        # If no exact alt-id match, try the top result if it's from Elsevier
        if items:
            top = items[0]
            doi = top.get("DOI", "")
            publisher = top.get("publisher", "").lower()
            if "elsevier" in publisher and validate_doi(doi):
                return doi
    except Exception:
        pass
    return None
...
def _scrape_identifier_from_webpage(url: str) -> Optional[Tuple[str, str]]:
    """Visit a URL and attempt to extract a structured identifier (DOI, arXiv ID,
    PMID, ISBN) from the page's HTML meta tags, link elements, and body text.
    This handles publisher pages like ScienceDirect, Wiley, Springer, SAGE, etc.
    that embed identifiers in meta tags (e.g. <meta name="citation_doi">).
    Returns (type, value) or None if no identifier is found."""
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        resp = requests.get(url, headers=headers, timeout=10)
        # Even on non-200, try to parse whatever HTML was returned (some sites
        # serve meta tags in 403/503 pages). Only bail on connection errors.
        soup = BeautifulSoup(resp.content, "html.parser")

        # --- 1. Look for DOI in meta tags (most common on publisher sites) ---
        doi_meta_selectors = [
            {"name": "citation_doi"},              # Google Scholar / most publishers
            {"name": "DOI"},                        # Some publishers
            {"name": "dc.identifier"},              # Dublin Core (often prefixed "doi:")
            {"name": "dc.Identifier"},              # Case variant
            {"name": "DC.identifier"},              # Case variant
            {"property": "citation_doi"},           # Alternate property form
            {"name": "prism.doi"},                  # PRISM metadata
        ]
        for selector in doi_meta_selectors:
            tag = soup.find("meta", attrs=selector)
            if tag and tag.get("content"):
                doi_candidate = tag["content"].strip()
                # Remove common prefixes like "doi:" or full URL
                doi_candidate = sanitize_doi(doi_candidate)
                doi_candidate = re.sub(r"^doi:\s*", "", doi_candidate, flags=re.IGNORECASE)
                if validate_doi(doi_candidate):
                    return "doi", doi_candidate

        # Check og:url or citation_abstract_html_url for doi.org links
        for prop in ["og:url", "citation_abstract_html_url", "citation_fulltext_html_url"]:
            tag = soup.find("meta", property=prop) or soup.find("meta", attrs={"name": prop})
            if tag and tag.get("content"):
                content = tag["content"].strip()
                doi_from_url = re.search(r"doi\.org/(10\.\d{4,9}/[^\s]+)", content)
                if doi_from_url:
                    doi_val = doi_from_url.group(1).rstrip("/")
                    if validate_doi(doi_val):
                        return "doi", doi_val

        # --- 2. Look for arXiv ID in meta tags ---
        arxiv_meta = soup.find("meta", attrs={"name": "citation_arxiv_id"})
        if arxiv_meta and arxiv_meta.get("content"):
            aid = sanitize_arxiv_id(arxiv_meta["content"].strip())
            if validate_arxiv_id(aid):
                return "arxiv", aid

        # --- 3. Look for PMID in meta tags ---
        pmid_meta = soup.find("meta", attrs={"name": "citation_pmid"})
        if pmid_meta and pmid_meta.get("content"):
            pid = pmid_meta["content"].strip()
            if validate_pmid(pid):
                return "pmid", pid

        # --- 4. Look for ISBN in meta tags ---
        isbn_meta = soup.find("meta", attrs={"name": "citation_isbn"})
        if isbn_meta and isbn_meta.get("content"):
            isbn = sanitize_isbn(isbn_meta["content"].strip())
            if validate_isbn(isbn):
                return "isbn", isbn

        # --- 5. Scan <a> tags and visible text for DOI patterns ---
        # Check link tags with DOI scheme
        for link in soup.find_all("a", href=True):
            href = link["href"]
            doi_in_href = re.search(r"doi\.org/(10\.\d{4,9}/[^\s\"]+)", href)
            if doi_in_href:
                doi_val = doi_in_href.group(1).rstrip("/")
                if validate_doi(doi_val):
                    return "doi", doi_val

        # --- 6. Regex scan of full page text as last resort ---
        page_text = soup.get_text(separator=" ")
        # Limit scan to first 5000 characters to avoid false positives in references
        page_text = page_text[:5000]

        doi_match = re.search(DOI_PATTERN, page_text, re.IGNORECASE)
        if doi_match:
            doi_val = sanitize_doi(doi_match.group(0))
            if validate_doi(doi_val):
                return "doi", doi_val

        arxiv_match = re.search(ARXIV_PATTERN, page_text, re.IGNORECASE)
        if arxiv_match:
            aid = sanitize_arxiv_id(arxiv_match.group(1))
            if validate_arxiv_id(aid):
                return "arxiv", aid

        pmid_match = re.search(PMID_PATTERN, page_text, re.IGNORECASE)
        if pmid_match:
            pid = pmid_match.group(1)
            if validate_pmid(pid):
                return "pmid", pid
...
def detect_identifier_type(identifier_input: str) -> Tuple[str, str]:
    ...
    if identifier_input.startswith(("http://", "https://", "www.")):
        ...
        if extracted:
            return extracted
        # URL pattern matching failed – visit the page and look for identifiers
        # in HTML meta tags and page content (handles publisher sites like
        # ScienceDirect, Wiley, Springer, SAGE, etc.)
        url = sanitize_url(identifier_input)
        if validate_url(url):
            scraped = _scrape_identifier_from_webpage(url)
            if scraped:
                return scraped
            # No identifier found even after scraping – treat as generic URL
            return "url", url
    # DOI
...
def _render_copyable_citation(citation_text: str, unique_key: str):
    ...
    # Theme detection: reads the Streamlit parent page's background color to
    # determine light vs dark mode (works regardless of OS-level preference).
    component_html = f"""
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        /* Dark mode – applied via JS-added class on <html> */
        html.dark-mode .citation-wrapper {{
        ...
        html.dark-mode .copy-btn:hover {{ background: #4a4b5a; }}
        .copy-btn {{
            position: absolute;
            top: 8px;
            right: 8px;
    ...
    <script>
        // Detect Streamlit theme by inspecting the parent page's background color.
        // Uses a retry loop because the parent DOM may not be fully rendered yet.
        function detectTheme_{unique_key.replace('-','_')}() {{
            try {{
                var targets = [
                ...
                for (var i = 0; i < targets.length; i++) {{
                    ...
                    var bgColor = window.parent.getComputedStyle(targets[i]).backgroundColor;
                    ...
                    if (match && match.length >= 3) {{
                        ...
                        if (luminance > 200) return true;  // Confirmed light mode
                    }}
                }}
            }} catch(e) {{
                // Cross-origin fallback: use prefers-color-scheme
        ...
        // Try immediately, then retry after short delays to handle slow renders
        if (!detectTheme_{unique_key.replace('-','_')}()) {{
            setTimeout(function() {{
                if (!detectTheme_{unique_key.replace('-','_')}()) {{
                    setTimeout(detectTheme_{unique_key.replace('-','_')}, 200);
                }}
            }}, 50);
        }}
