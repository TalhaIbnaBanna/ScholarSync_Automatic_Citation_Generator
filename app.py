...
def get_metadata_from_pubmed(pmid: str) -> Optional[Dict[str, Any]]:
    """Fetch article metadata from PubMed's ESummary API using a numeric PMID.
    Returns title, authors, year, DOI, journal name, volume, issue, and pages."""
...
def format_citation_from_metadata(metadata: Dict[str, Any], style_name: str) -> str:
    ...
    Defaults to journal article formatting if no type is specified."""
    try:
...
def export_references_to_word(
    ...
    try:
        ...
        def _add_citation(para, citation_text, meta, style):
            ...
            for journal names and volume numbers (as required by APA/Harvard)."""
